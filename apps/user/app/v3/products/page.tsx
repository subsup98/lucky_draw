...
export default function V3ProductsPage() {
  ...
  useEffect(() => {
    ...
    setErr(null);
    withDevFallback(api<StoreProduct[]>(`/api/products${qs}`), filterDemoProducts(type))
      .then(setProducts)
    ...
    <div className="min-h-screen bg-neutral-50 text-neutral-950">
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
        <div className="mb-5 flex flex-col gap-3 border-b border-neutral-200 pb-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-sm font-black text-red-600">V3 ONLINE SHOP</p>
            <h1 className="mt-1 text-3xl font-black tracking-normal">상품</h1>
            <p className="mt-2 text-sm text-neutral-600">
              공개 상품 API를 기준으로 예약 구매와 일반 판매 상품을 탐색합니다.
            </p>
          </div>
        ...
        {filteredProducts.length > 0 && (
          <ul className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5">
            {filteredProducts.map((product) => (
              <ProductCard key={product.id} product={product} productBasePath="/v3/products" />
            ))}
          </ul>
        )}
      </main>
