import { Stack, Link } from "expo-router";
import {
  ActivityIndicator,
  FlatList,
  Pressable,
  Text,
  View,
} from "react-native";
import { SafeAreaView } from "react-native-safe-area-context";
import { useOrders } from "../../lib/hooks";
import { ORDER_STATUS_LABEL, formatDate } from "../../lib/labels";
import { apiErrorToKo } from "../../lib/error-message";

export default function OrdersListScreen() {
  const orders = useOrders();

  return (
    <SafeAreaView className="flex-1 bg-white">
      <Stack.Screen
        options={{ headerShown: true, title: "내 주문", headerBackTitle: "뒤로" }}
      />
      <FlatList
        data={orders.data ?? []}
        keyExtractor={(item) => item.id}
        contentContainerStyle={{ paddingVertical: 8 }}
        renderItem={({ item }) => (
          <Link
            href={{ pathname: "/orders/[id]", params: { id: item.id } } as never}
            asChild
          >
            <Pressable className="px-4 py-3 border-b border-gray-100 active:bg-gray-50">
              <View className="flex-row justify-between items-center mb-1">
                <Text className="text-sm font-medium">
                  티켓 {item.ticketCount}장
                </Text>
                <Text className="text-xs text-gray-500">
                  {ORDER_STATUS_LABEL[item.status] ?? item.status}
                </Text>
              </View>
              <Text className="text-xs text-gray-500">
                {item.totalAmount.toLocaleString()}원 · {formatDate(item.createdAt)}
              </Text>
            </Pressable>
          </Link>
        )}
        ListEmptyComponent={
          orders.isLoading ? (
            <View className="items-center py-10">
              <ActivityIndicator />
            </View>
          ) : orders.error ? (
            <Text className="text-center text-sm text-red-500 py-10">
              {apiErrorToKo(orders.error)}
            </Text>
          ) : (
            <Text className="text-center text-sm text-gray-500 py-10">
              주문 내역이 없습니다.
            </Text>
          )
        }
      />
    </SafeAreaView>
  );
}
